...
def getfirstData(grade,positionName,year,month,comsize):
    ...
    datestr = year + '-%'+month+"-%"
    if year and month:
        sql='select count(*) from jobdata where grade= %s and positionName like %s and createTime like %s and avgSalary>0 and companySize=%s and industry!=""'
        cursor.execute(sql,(grade,positionName,datestr,comsize))    #执行sql语句  
    else:
        sql='select count(*) from jobdata where grade= %s and positionName like %s and avgSalary>0 and companySize=%s and industry!=""'
...
def getComSizeNeed(request):
    ...
    try:
        ...
        for i in gradeArray:
            ...
            seriesChild={}
            for j in gradeName:
                count = getfirstData(i,positionName,year,j,comsize)
